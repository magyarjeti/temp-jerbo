...
export const ColumnBlock = Node.create<ColumnBlockOptions>({
		...
		return {
			columns: {
				...
				renderHTML(attributes) {
					return {
						style: `grid-template-columns: repeat(${attributes.columns}, minmax(0, 1fr))`,
					};
				},
			},
		};
	...
	renderHTML({ HTMLAttributes }) {
		const attrs = mergeAttributes(HTMLAttributes, { class: 'grid gap-3' });
		return ['div', attrs, 0];
	},
			...
			({ tr, dispatch }: CommandProps) => {
				try {
					...
					if (keepContent) {
						...
						columnBlock = buildColumnBlock({ content: columns, n });
					}
